package ca.uqtr.tp2.Reflection;
...
import ca.uqtr.tp2.Persistence.metadata.TableInfo;
...
public class ReflectionManager {
	...
	public static List<ColumnInfo> getColumns(Class<?> bean) {
		...
		.filter(f-> !hasIgnoreAnnotation(f))
	...
	public static List<JoinInfo> getRelations(Class<?> beans, boolean excludeIgnore) {
		...
		.filter(field -> {
			...
			return !hasIgnoreAnnotation(field);
		})
		.filter(method -> method.isAnnotationPresent(Join.class))
		.forEach(field -> {
			try {
				// if(!field.getClass().equals(beans.getClass()))
